...
void FlatpakApi::SetUp(
  ...
  {
    BasicMessageChannel<> channel(binary_messenger, "dev.flutter.pigeon.flatpak_flutter.FlatpakApi.setupEventChannel" + prepended_suffix, &GetCodec());
    if (api != nullptr) {
      channel.SetMessageHandler([api](const EncodableValue& message, const flutter::MessageReply<EncodableValue>& reply) {
        try {
          const auto& args = std::get<EncodableList>(message);
          ...
          api->SetupEventChannel(app_id_arg, [reply](std::optional<FlutterError>&& output) {
            if (output.has_value()) {
